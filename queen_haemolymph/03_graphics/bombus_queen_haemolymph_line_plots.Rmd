...
lfq_values <- read.table(file = "input/input_for_plots.txt", header = TRUE)
...
lfq_values_subset_log <- log10(lfq_values_subset)

## Need to be able to subset genes of interest:
amp_proteins <- read.table(file = "data/amp_list.txt",
                              header = FALSE)

## Update column names:
colnames(amp_proteins) <- c("description", "protein_id")

## Subset proteins of interest:
tmp <- subset(lfq_values_subset_log, row.names(lfq_values_subset_log)
              %in% amp_proteins$protein_id)

## Add protein description:
tmp$description <- amp_proteins$description
