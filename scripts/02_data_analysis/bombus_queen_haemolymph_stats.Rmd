...
for (protein in 1:(ncol(haem_data_df) - 1)){
        res_aov <- aov(haem_data_df[, protein] ~ life_stages,
                       data = haem_data_df)
        anova_df <- rbind(unlist(summary(res_aov)), anova_df)
...
## Write to file:
write.table(anova_df, file = "./results/anova_output.txt")
write.table(tukey_output_df_sig, file = "./results/tukey_output.txt")
```
